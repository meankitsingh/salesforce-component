...
exports.getJSON = async function getJSON(logger, params) {
  ...
  logger.trace('Sending %s request to %s', method, uri);
  ...
  let response;
  try {
    response = await request[method.toLowerCase()](parameters);
  } catch (err) {
    logger.error(`Failed to fetch JSON from ${uri} with error: ${err}`);
    throw err;
  }
  ...
  if (statusCode === expectedStatus) {
    ...
    if (result) {
      logger.trace('Have got %d response from %s to %s', expectedStatus, method, uri);
    } else {
      logger.info('Have got empty response');
    }
    return result;
  }
  const msg = util.format('Unexpected return code %d, expected %d, body %j', statusCode, expectedStatus, body);
  logger.error(msg);
